...
namespace AsdRcSlab
{
    ...
    public static class BS8666Calculator
    {
        ...
        public static UsedDimensions GetUsedDimensions(int code)
        {
            ...
            {
                case 0:
                    // Straight: special case w writerze (A="STR" string),
                    // tutaj zwracamy puste — writer ma osobną ścieżkę.
                    return new UsedDimensions();

                // A + B only
                case 11: case 47: case 51: case 63: case 75:
                    return new UsedDimensions { HasA = true, HasB = true };

                // A + C only (no B, no D, no E)
                case 14: case 15:
                    return new UsedDimensions { HasA = true, HasC = true };

                // A + B + C (no D, no E)
                case 13: case 21: case 23: case 24: case 26:
                case 27: case 28: case 29: case 33: case 77:
                    return new UsedDimensions {
                        HasA = true, HasB = true, HasC = true };

                // A + B + C + D (no E)
                case 22: case 31: case 32: case 36: case 98:
                    return new UsedDimensions {
                        HasA = true, HasB = true, HasC = true, HasD = true };

                // A + B + E (no C, no D)
                case 12: case 25:
                    return new UsedDimensions {
                        HasA = true, HasB = true, HasE = true };

                // A + B + C + E (no D)
                case 34: case 35: case 46:
                    return new UsedDimensions {
                        HasA = true, HasB = true, HasC = true, HasE = true };

                // A + B + C + D + E
                case 41: case 44: case 56:
                    return new UsedDimensions {
                        HasA = true, HasB = true, HasC = true,
                        HasD = true, HasE = true };
                ...
                // A only
                case 67:
                    return new UsedDimensions { HasA = true };

                default:
                    // Nieznany shape code — safe fallback, nic nie wpisuj.
                    return new UsedDimensions();
            }
        }
    }
